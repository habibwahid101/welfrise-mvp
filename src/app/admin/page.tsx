...
export default async function AdminPage() {
  ...
  return <main className="admin-page"><div className="admin-wrap">
    ...
    {queryError ? <div className="notice error" role="alert">Some admin data is temporarily unavailable. Confirm migrations 001, 002, and 003 are applied.</div> : null}
    ...
    <section className="panel"><h2>Binance payment reviews</h2><p className="small-muted">A screenshot is supporting evidence only. Approval requires a successful independent on-chain match against the configured chain, contract, recipient, amount, and confirmation policy.</p><div className="table-scroll"><table><thead><tr><th>Participant</th><th>Request</th><th>Transaction & proof</th><th>Status</th><th>Verification</th></tr></thead><tbody>{signedBinance.map((item) => <tr key={item.id}><td>{String(item.participant_name || item.participant_email || 'User')}</td><td>Level {item.level_id}<br />{money(item.amount)} · {item.slots} slots<br /><MaskedValue value={String(item.assigned_wallet_address || '')} /></td><td>{item.tx_hash ? <MaskedValue value={String(item.tx_hash)} /> : 'Not submitted'}<br />{item.proofUrl ? <a href={item.proofUrl} target="_blank" rel="noreferrer">View private proof</a> : mutationEnabled ? '—' : 'Verify MFA to view proof'}</td><td>{statusBadge(item.status)}</td><td>{['submitted','held'].includes(item.status) ? <BinanceReviewForm id={item.id} amount={String(item.amount)} address={String(item.assigned_wallet_address || '')} enabled={mutationEnabled} /> : '—'}</td></tr>)}</tbody></table></div></section>

    <section className="panel"><h2>User Wallet authorization activity</h2><div className="table-scroll"><table><thead><tr><th>Participant</th><th>Wallet owner</th><th>Level/package</th><th>Status</th><th>Created</th></tr></thead><tbody>{userWalletRequests.map((item) => { const participant = joined(item.participant); const payer = joined(item.payer); return <tr key={item.id}><td>{String(participant?.full_name || participant?.email || 'Participant')}</td><td>{String(payer?.full_name || payer?.email || 'Wallet owner')}</td><td>Level {item.level_id}<br />{money(item.amount)} · {item.slots} slots</td><td>{statusBadge(item.status)}</td><td>{new Date(item.created_at).toLocaleString()}</td></tr> })}</tbody></table></div></section>
