  ...
  return (
    <main className="portal-page">
      <header className="portal-head">
        ...
        <nav className="portal-nav"><Link href="/app">Dashboard</Link><Link href="/app/payments">Payments & Wallet</Link><Link href="/account/security">Account Security</Link></nav>
      </header>
      <div className="portal-stack">
        <section className="portal-panel">
          <div className="panel-title-row"><div><h2>Identity verification</h2>{!canSubmit ? <p>Status: <strong>{labels[status] || status.replaceAll('_', ' ')}</strong></p> : null}</div></div>
          ...
          {!canSubmit ? <div className="notice success" role="status">{status === 'approved' ? 'Your identity verification is approved.' : 'Your submission is being reviewed. You cannot replace documents while review is active.'}</div> : <KycForm canSubmit={canSubmit} initialStatus={labels[status] || status.replaceAll('_', ' ')} />}
        </section>
      </div>
    </main>
  )
