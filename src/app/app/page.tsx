...
export default async function AppPage() {
  ...
  const [profileResult, walletResult, slotsResult, ledgerResult, incomingResult, outgoingResult, noticesResult, referrerResult] = await Promise.all([
    ...
    supabase.from('wallet_payment_requests').select('id,participant_display,amount,level_id,status,expires_at').eq('payer_id', user.id).eq('status', 'pending').order('created_at', { ascending: false }).limit(8),
    supabase.from('wallet_payment_requests').select('id,payer_display,amount,level_id,status,created_at').eq('participant_id', user.id).eq('status', 'pending').order('created_at', { ascending: false }).limit(8),
    ...
    <main className="portal-page">
      ...
      <div className="portal-stack">
        ...
        <section className="portal-grid-two">
          <div className="portal-panel"><h2>Pending incoming wallet authorizations</h2><div className="compact-list">{(incomingResult.data || []).map((item) => <div key={item.id}><span>{item.participant_display}<small>Level {item.level_id} · expires {date(item.expires_at)}</small></span><strong>{money(item.amount)}</strong></div>)}{!incomingResult.data?.length ? <p className="empty-copy">No requests need your approval.</p> : null}</div><Link className="text-link" href="/app/payments">Review authorizations</Link></div>
          <div className="portal-panel"><h2>Pending outgoing requests</h2><div className="compact-list">{(outgoingResult.data || []).map((item) => <div key={item.id}><span>{item.payer_display}<small>Level {item.level_id} · {date(item.created_at)}</small></span><strong>{money(item.amount)}</strong></div>)}{!outgoingResult.data?.length ? <p className="empty-copy">No pending outgoing requests.</p> : null}</div><Link className="text-link" href="/app/payments">Manage requests</Link></div>
        </section>
        <section className="portal-grid-two">
